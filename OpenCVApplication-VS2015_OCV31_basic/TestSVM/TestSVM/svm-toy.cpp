	...
	case WM_COMMAND:
		{
			...
			case ID_BUTTON_SIMPLE_CLASSIFIER:
				...
				int nrSamples1 = 0;
				int nrSamples2 = 0;
				double xC1sum = 0, xC2sum = 0, yC1sum = 0, yC2sum = 0;
				for (p = point_list.begin(); p != point_list.end(); p++)
				{
					if ((*p).value == 1) //point from class ‘1’
					{
						nrSamples1++;
						xC1sum += (*p).x; //normalized x coordinate of the current point
						yC1sum += (*p).y; //normalized y coordinate of the current point
					}
					if ((*p).value == 2) //point from class ‘2’
					{
						nrSamples2++;
						xC2sum += (*p).x; //normalized x coordinate of the current point
						yC2sum += (*p).y; //normalized y coordinate of the current point
					}
				}
				point cPlus = { 0.0,0.0,0 },
					cMinus = { 0.0,0.0,0 },
					c = { 0.0,0.0,0 },
					w = { 0.0,0.0,0 };

				cPlus.x = xC1sum / nrSamples1;
				cPlus.y = yC1sum / nrSamples1;

				cMinus.x = xC2sum / nrSamples2;
				cMinus.y = yC2sum / nrSamples2;

				c.x = (cPlus.x + cMinus.x) / 2.0;
				c.y = (cPlus.y + cMinus.y) / 2.0;

				w.x = cPlus.x - cMinus.x;
				w.y = cPlus.y - cMinus.y;
				
				double b = (cPlus.x*c.x) + (cPlus.y*c.y) - ( (cMinus.x*c.x) + (cMinus.y *c.y));
				 
				//ax + by - c = 0 <=> < (w.x, w.y), (x, y) > - b = 0, <> =  dot product
				// w contains the a,b parameters
 
				DrawLine(window_dc, 0, b/w.y,XLEN,(b-XLEN*w.x)/w.y, RGB(255, 0, 0));
				DrawLine(window_dc, b/w.x , 0, (b -(w.y*YLEN))/w.x , YLEN, RGB(0, 255, 0));
