...
Point* computeEndPointsFromPointSet(Point* points, int numberOfPoints) {
	Point* endPoints = new Point[2];
	int xMax = -10000, xMin = 100000;
	for (int i = 0; i < numberOfPoints; i++) {
		if (points[i].y > xMax) {
			xMax = points[i].y;
			endPoints[0].x = points[i].x;
			endPoints[0].y = points[i].y;
		}
		if (points[i].y < xMin) {
			xMin = points[i].y;
			endPoints[1].x = points[i].x;
			endPoints[1].y = points[i].y;
		}
	}
	return endPoints;
}
...
void RANSAC_lab02() {
 	Mat img = imread("points_RANSAC/points5.bmp", CV_LOAD_IMAGE_GRAYSCALE);
	int height = img.rows;
	int width = img.cols;
	//assume 1000 points
	...
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width; j++){
			if (img.at<uchar>(i, j) == 0) {
				points[numberOfPoints].x = j;
				points[numberOfPoints].y = i;
				numberOfPoints++;
			}
		}
	}
	...
	float q = 0.9f;
	float s = 2.0f;
	float N = log(1.0f - q) / log(1.0f - pow(q,s));
	float T = q*numberOfPoints;
	//create matrix of points in order to store inliers for each try
	Point** inLiers = (Point**)malloc(ceil(N) * sizeof(Point*));
	int numberOfInLiers = 0;
	for (int row = 0; row < ceil(N); row++) {
		inLiers[row] = (Point*)malloc(1000 * sizeof(Point));
	}
	//line equation of form : aX +bY +c = 0
	abcLine eq_line;
	...
	for (int tries = 0; tries < N; tries++) {
		//reset count of inliers for each count
		numberOfInLiers = 0;
		//get a new points sample for each try
		srand(time(NULL));
		int pointIndexOne = rand() % numberOfPoints;
		int pointIndexTwo = rand() % numberOfPoints;

		pointOne = points[pointIndexOne];
		while (pointIndexOne == pointIndexTwo)
			pointIndexTwo = rand() % numberOfPoints;
		pointTwo = points[pointIndexTwo];

		eq_line.a = pointOne.y - pointTwo.y;
		eq_line.b = pointTwo.x - pointOne.x;
		eq_line.c = pointOne.x*pointTwo.y - pointTwo.x*pointOne.y;
		for (int k = 0; k < numberOfPoints; k++) {
			float dist = abs(points[k].x * eq_line.a + points[k].y*eq_line.b + eq_line.c) / sqrt(eq_line.a*eq_line.a + eq_line.b*eq_line.b);
			if (dist < t) {
				inLiers[tries][numberOfInLiers] = points[k];
				numberOfInLiers++;
				//remember the try with most inliers and  its index
				if (tries == 0) {
					maxInliersCount = numberOfInLiers;
					maxInliersCount = tries;
				}
			}
		}
		std::cout << "Iteration " << tries << ": " << pointIndexOne << " <-indexes->" << pointIndexTwo;
		std::cout << " Inlier: " << numberOfInLiers << " Threshold: " << T << "\n";
		waitKey(0);
		if (numberOfInLiers > T) {
		//re estimate model
			Point *endPoints = computeEndPointsFromPointSet(inLiers[tries], numberOfInLiers);
			line(img, endPoints[0], endPoints[1], Scalar(0, 0, 255));
			imshow("Inliers > threshold", img);
			drewLine = true;
			waitKey(0);
			break;
		}
			...
			if (numberOfInLiers > maxInliersCount) {
				maxInliersCount = numberOfInLiers;
				maxInliersIndex = tries;
			}
		}
		waitKey(0);
	}
	//after trying fo N times, go back to the estimation with the max number of inliers
	//and reestimate the model
	if (drewLine == false) {
		Point* endpoints = computeEndPointsFromPointSet(inLiers[maxInliersIndex], maxInliersCount);
		line(img, endpoints[0], endpoints[1], Scalar(0, 0, 255));
		imshow("Best Inliers Ser", img);
		std::cout << maxInliersCount;
		waitKey(0);
	}
...
void houghTransform_lab03() {
	Mat srcImg = imread("images_hough/image_simple.bmp", CV_LOAD_IMAGE_GRAYSCALE);
	...
	for (int i = 0; i < imageWithEdges.rows; i++) {
		...
		//detect edge point
			if (imageWithEdges.at<uchar>(i, j) == 255) {
				...
				for (int theta = 0; theta < maxTheta; theta += deltaTheta) {
					//p=xcos(theta) + ysin(theta)
					p = j * cos(theta) + i * sin(theta);
					if (p >= 0 && p <= maxP) {
	...
	imshow("houghImgNormalized", houghImg);
	...
	for (int i = 0; i < 10; i++) {
		Point pt1, pt2;
		double a = cos(peaks[i].theta), b = sin(peaks[i].theta);
...
//Chamfer based DT, 3x3 mask splitted in two
/***********************************NOT WORKING******************************************************************/
Mat distanceTransform_lab04() {

	Mat srcImg = imread("images_DT_PM/DT/contour1.bmp", IMREAD_GRAYSCALE);
	Mat DT = srcImg.clone();
	...
	//set up image
	Mat im_th;
 	threshold(srcImg, im_th, 220, 255, THRESH_BINARY_INV);
	Mat im_floodfill = im_th.clone();
	floodFill(im_floodfill, cv::Point(0, 0), Scalar(255));
	Mat im_floodfill_inv;
	bitwise_not(im_floodfill, im_floodfill_inv);
	//"object pixels are black, background pixels are white"
	for (int i = 0; i < height ; i++) {
		for (int j = 0; j < width; j++) {
			im_floodfill_inv.at<uchar>(i, j) = 255 - im_floodfill_inv.at<uchar>(i, j);
		}
	}
	
	// must be fixed 
	// toDo: ask for someone to debug code
	 //first scan
	 int vals[4];
	 int min;
	 int min2;

	 Mat buf = srcImg.clone();
	 for (int l = 0; l < 15; l++) {
		 for (int i = 1; i < height - 1; i++) {
			 for (int j = 1; j < width - 1; j++) {
				 ...
				 for (int k = 0; k < 4; k++) {
					 vals[k] = 0;
					 vals[k] = (int)buf.at<uchar>(i + di[k], j + dj[k]) + weight[k];
				 ...
				 // min = getMin(vals[0], vals[1], vals[2], vals[3]);
				 if (min2 < 255) {
					 DT.at<uchar>(i, j) = min2;
				 }
				 else {
					 DT.at<uchar>(i, j) = 255;
				 }
		 ...
		 for (int i = height - 2; i >= 1; --i) {
			 for (int j = width - 2; j >= 1; --j) {
				 ...
				 for (int k = 4; k < 8; k++) {
					 vals[k - 4] = 0;
					 vals[k - 4] = (int)buf.at<uchar>(i + di[k], j + dj[k]) + weight[k];
...
void statisticalAnalysis_lab05()
{
	...
	//since the feature vectors have 19x19 dimension
	//on each column we can compute the mean for that dimension among all feature vectors
 
 	float meanValues[N];
	for (int i = 0; i < N; i++) {
		float miu = 0.0f;
		for (int k = 0; k < p; k++) {
			miu += (float)I.at<uchar>(k, i);
		}
		miu = miu / (float)p;
		meanValues[i] = miu;
	}
	...
	//std deviation for each feature(image)
	float stdDevValues[N];
	for (int i = 0; i < N; i++) {
		float stdDev = 0.0f;
		for (int k = 0; k < p; k++) {
			stdDev += ((float)I.at<uchar>(k, i) - meanValues[i]) * ((float)I.at<uchar>(k, i) - meanValues[i]);
		}
		stdDev = stdDev / (float)(p-1);
		stdDevValues[i] = sqrt(stdDev);
	}
	//covariance matrix
	...
	//correlation chart = 256x256 white img
	Mat corChart = Mat(256, 256, CV_8UC1);
	for (int i = 0; i < 256; i++) {
		for (int j = 0; j < 256; j++) {
			corChart.at<uchar>(i, j) = 255;
		}
	}
	//(5,4) , (5,14) 
	for (int k = 0; k < p; k++) {
			...
			corChart.at<uchar>(r,c) = 0;
	}
	std::cout<<cor.at<float>((5*19)+4, (5 * 19)+14)<<std::endl;
	
	imshow("correlationChart", corChart);
	waitKey(0);

	//plot pdf 
	//G(x) =( 1 / 2*pi*std_dev  ) * exp(- ( (x-mean)^2 )/2*std_dev^2 )
...
void KMeansClustering_labo06_setOfPoints(int noOfIterations) {
	// d = 2 ; 2D points
	Mat srcImg = imread("images_Kmeans/points4.bmp", CV_LOAD_IMAGE_GRAYSCALE);
	Mat srcImgRgb(srcImg.size(), CV_8UC3);
	cvtColor(srcImg, srcImgRgb, CV_GRAY2RGB);
	...
	imshow("result", srcImgRgb);
	imshow("voroni_mozaicare", voroni);
	waitKey(0);
...
void bayesian_classifier_lab09() {
	...
	for (int count = 0; count < noOfTestInstances; count++) {
		...
		C.at<float>(predictedClass, (int)yTest.at<uchar>(count))+=1.0f;
	}
